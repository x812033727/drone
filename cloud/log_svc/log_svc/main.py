...
- POST /api/v1/logs/{drone_id}(multipart file)→ 存 /data/ulog/{drone_id}/
  ...
  同名 .report.txt,摘要列(report_ok + 前 500 字)落 flight_logs 表。
...
from log_svc.report import excerpt, run_report
...
INSERT_SQL = (
    "INSERT INTO flight_logs (time, drone_id, filename, size_bytes, report_ok, report_excerpt) "
    "VALUES (now(), $1, $2, $3, $4, $5)"
)
LIST_SQL = (
    "SELECT time, filename, size_bytes, report_ok FROM flight_logs "
...
async def process_log(drone_id: str, path: Path, size_bytes: int) -> None:
    ...
    await app.state.pool.execute(
        INSERT_SQL, drone_id, path.name, size_bytes, report_ok, excerpt(text)
    )
    log.info(
        "已落庫 flight_logs:%s/%s(%d bytes,report_ok=%s)",
        drone_id,
        path.name,
        size_bytes,
        report_ok,
