...
_jwks_client …)驗證,這些狀態必須留在被 patch 的服務模組內,不可抽離。
...
def authorize_token(token: str | None, min_role: str) -> dict:
    """驗證 raw token 字串並檢查角色,回 claims。dev 模式(認證停用)放行為 admin。"""
    if not AUTH_ENABLED:
...
# 純邏輯自 drone_common.auth 再匯出,保留 `from mission_svc.auth import ...` 既有路徑不變。
__all__ = [
    "AUTH_ENABLED",
    "DEV_ORG",
    "ROLE_ORDER",
