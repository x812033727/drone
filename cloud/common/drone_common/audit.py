"""共用審計軌跡寫入(G14)。變更端點成功後旁路記一筆到 <schema>.audit_log。
schema 由呼叫端參數化(fleet.audit_log / mission.audit_log)。Wave 1 A1 去重。
...
- SQL 寫在此(小 helper),查詢(list/count)走各服務 repo.py,與分頁慣例一致。
"""
...
log = logging.getLogger("drone_common.audit")

_RANK_TO_ROLE = {rank: name for name, rank in ROLE_ORDER.items()}


def make_insert(schema: str) -> str:
    """組出寫審計的 INSERT 語句(schema 為服務內建常數,非外部輸入)。"""
    return (
