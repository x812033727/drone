...
-- 定期 DELETE FROM fleet.rate_limit_counter WHERE window_start < <now-緩衝>(非必要;
-- window_start 索引支援)。

CREATE TABLE IF NOT EXISTS fleet.rate_limit_counter (
...
CREATE INDEX IF NOT EXISTS rate_limit_counter_window_idx
    ON fleet.rate_limit_counter (window_start);
