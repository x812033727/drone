...
參數檔與批次寫入/核對工具:[tools/flight_ops](../../../tools/flight_ops/README.md)(`--dry-run` 供 §7 飛行日參數核對)。
...
| `COM_LOW_BAT_ACT` | 3(Critical 返航、Emergency 降落) | 低電量分級:Low 警告 → Critical 返航 → Emergency 降落(PX4 v1.15 中 2=Land mode 會讓 Critical 直接就地降落,SITL 場景 F10 實測確認,故用 3) |
...
| 振動初查 | 懸停 1 min 的 ULog,`ulog_report.py` 看加速度 FFT | 槳頻峰值於 EKF 濾波帶外(思路引 [propulsion.md §9](../../10-hardware/propulsion.md)) |
