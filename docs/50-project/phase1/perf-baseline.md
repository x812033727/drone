...
|------|------|--------|-----|-----|--------------|---------|------|
| 2026-07-17 | 共用開發機(此機同時跑多個正式服務,非隔離基準機;8 vCPU/隔離 compose 棧) | 20 | 5 ms | 100 ms | 0 | 0(dev-secret admin 路徑不觸發) | locust 90s，1923 req,吞吐 21.6 req/s;p99=100ms |

### 3.2 遙測 fan-in(mqtt_fanin)

| 日期 | 環境 | drones×rate | 實發 msg/s | 落庫率 | 備註 |
|------|------|-------------|-----------|--------|------|
| 2026-07-17 | 共用開發機(此機同時跑多個正式服務,非隔離基準機;8 vCPU/隔離 compose 棧) | 100×5 | 489 msg/s(目標 500) | **100%**(22500/22500) | 45s;瓶頸在同機 TimescaleDB 寫入而非 broker |

### 3.3 SSE 訂閱量(sse_swarm,後續 PR)

| 日期 | 環境 | 訂閱者數(正常/慢讀/斷線) | fd(結束) | RSS | 斷線後 gauge 歸零 | 備註 |
|------|------|---------------------------|---------|----------|-------------------|------|
| 2026-07-17 | 共用開發機(此機同時跑多個正式服務,非隔離基準機;8 vCPU/隔離 compose 棧) | 300(180/60/60 RST) | 16 | 77 MB | ✅ final_gauge=0 | 300/300 連線成功;fd 回落基線=無洩漏 |

### 3.4 混沌演練恢復(chaos_drill.sh)

| 日期 | 環境 | 場景 | 結果 | 備註 |
|------|------|------|------|------|
| 2026-07-17 | 同上 | S1 DB 重啟 | ✅ pool 自癒,ingest restarts=0 | — |
| 2026-07-17 | 同上 | S2 DB 長停 | ✅ DLQ 落地 5 行 + 恢復續落庫 | — |
| 2026-07-17 | 同上 | S3 MQTT 重啟 | ✅ 重連恢復,dispatch 斷線回乾淨 HTTP 500 | — |

三場景全程 **~55s**(含起棧),自癒無需容器重啟。
...
| 2 | 2026-07-17 | 回填首批實測基準(REST p50=5ms/p95=100ms、fan-in 100 機 489 msg/s 落庫 100%、SSE 300 無洩漏、chaos 三場景 ~55s);共用開發機非隔離基準機,數字僅供迴歸對照 |
