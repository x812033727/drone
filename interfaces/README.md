...
| `fleet/{drone_id}/cmd/mission_ctrl` | `MissionCommand` | 雲 → 機 | 事件觸發,QoS 1(v0.2.0 預留契約,proto3 JSON;機上訂閱與執行 **S12 實作**) |
...
- 版本號記錄於 `proto/gen/python/pyproject.toml`(目前 0.3.0)
...
### v0.5 規劃預告(方向宣告,不動 proto 檔)

雲端任務派遣協議(契約草案見 [cloud-fleet.md §6](../docs/20-software/cloud-fleet.md))
預計於 **v0.5** 落地為 proto 訊息,方向:

- `FleetMission`:雲端派遣單(工單層,不上線下發;schema 供 mission-svc 與
  Web 指揮中心共用)——生命週期 CREATED/ASSIGNED/EXECUTING/COMPLETED/CANCELLED
- `MissionAssignment`:指派事件(派遣單 ↔ drone_id 綁定與前置檢查結果),
  供多機調度審計與追溯
- 機上契約**不變**:飛機側仍只認 `MissionPlan` / `MissionProgress` /
  `MissionCommand`,`mission_id` 貫穿派遣單與機上執行,為端到端追溯鍵
- 去重/冪等沿用 QoS 1 at-least-once 既定語意(cloud-fleet §6.4);純增量 = MINOR

> 本節僅為規劃預告;v0.4 版號預留給先到的增量需求,實際訊息定義以屆時 PR 為準。
