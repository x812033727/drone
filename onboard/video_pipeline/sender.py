#!/usr/bin/env python3
"""時戳嵌入推流端:合成畫面 → H.264(x264 zerolatency)→ RTSP 推 MediaMTX。

每幀在左上角時戳條寫入當下 unix_time_ms(見 stamp.py),其餘畫面為
彩條背景 + 移動方塊 + 每幀更新的雜訊帶(讓編碼器有真實工作量,
避免靜態畫面把編碼成本量測得過於樂觀)。
...
def build_pipeline(args) -> tuple[Gst.Pipeline, Gst.Element]:
    desc = (
        ...
        f"caps=video/x-raw,format=I420,width={args.width},height={args.height},"
        f"framerate={args.fps}/1 "
        f"! videoconvert "
        f"! x264enc tune=zerolatency speed-preset=ultrafast bitrate={args.bitrate} "
        f"key-int-max={args.fps} bframes=0 "
        f"! video/x-h264,profile=constrained-baseline "
        f"! h264parse "
        f"! rtspclientsink name=sink protocols=tcp location={args.rtsp_url}"
    )
